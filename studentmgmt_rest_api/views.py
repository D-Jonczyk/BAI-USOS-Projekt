...
def get_student_courses(request):
    try:
        # student = Student.objects.get(user_id=request.user.id) # bezpieczny kod

        user_id = request.GET.get('user_id')  # This line is intentionally vulnerable
        if user_id:
            with connection.cursor() as cursor:
                cursor.execute(f"SELECT * FROM student_courses WHERE user_id = {user_id}")
                rows = cursor.fetchall()
            # Assuming 'EnrollmentSerializer' can handle raw query results:
            serializer = EnrollmentSerializer(rows, many=True)
            return Response(serializer.data)
        else:
            # Fallback to original secure code if 'user_id' is not provided
...
# podatna na SQL injection funkcja
